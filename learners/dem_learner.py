...
class DEMLearner(Learner):
    ...
    def before_task_learning(self, tid):
        ...
        task_detector_num = len(self.model.task_gaes) if self.args.task_free else 0
    ...
    def task_learning(self, tid, train_task, val_task):
        ...
        # learning GAE for current task
        if self.args.task_free and not self.args.no_gae_train:
            self.gae_learning(tid, train_task, val_task)
    ...
    def gae_learning(self, tid, train_task, val_task):
        logging.info("[GAE] Starting to train GAE for task_{}".format(tid))
        gae = self.model.task_gaes[-1]
        gae.train()
        optimizer = torch.optim.SGD(gae.parameters(), lr=self.args.gae_lr)
        if self.args.use_lrschd:
            scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=self.args.lr_sh_rate, gamma=0.2)
    
        self.gae_metrics = {'train_loss':[],'val_loss':[]}
        self.gae_constant_metrics = {'min_val_epoch':-1, 'min_val_loss':9999}
        self.gae_best_model = None
        ...
        for ep_id in range(self.args.num_epochs):
            # training
            gae, train_loss = train(gae, optimizer, train_loader, self.args.batch_size)
            self.gae_metrics['train_loss'].append(train_loss)
            # validating
            val_loss = valid(gae,val_loader,self.args.batch_size)
            self.gae_metrics['val_loss'].append(val_loss)
            if torch.isnan(torch.tensor(train_loss)).any():
                logging.warning("[GAE-Loss] Loss contains NaN values.")
            
            # save best GAE model 
            if val_loss < self.gae_constant_metrics['min_val_loss']:
                logging.info("[GAE-Loss] Epoch {}, train_loss: {:.4f}, val_loss: {:.4f}".format(ep_id,train_loss,val_loss))
                self.gae_constant_metrics['min_val_loss'] = val_loss
                self.gae_constant_metrics['min_val_epoch'] = ep_id
                self.gae_best_model = copy.deepcopy(gae)
                # self.store_checkpoint(self.best_model, self.extra_info)
                # save GAE separately
                torch.save(gae.state_dict(),"./models/task_detector/checkpoint/GAE_task_{}.pth".format(tid))
            
            if ep_id - self.gae_constant_metrics['min_val_epoch'] > self.args.early_stop_epochs:
                break
            
            if self.args.use_lrschd:
                scheduler.step()
        
        self.model.task_gaes[-1] = self.gae_best_model
        
        # save loss
        if self.args.store_loss:
            np_val_loss = np.array(self.gae_metrics['val_loss'])
            np_tra_loss = np.array(self.gae_metrics['train_loss'])
            np.save(self.root_path+'/loss/gae_val_task_{}.npy'.format(tid), np_val_loss)
            np.save(self.root_path+"/loss/gae_train_task_{}.npy".format(tid), np_tra_loss)
